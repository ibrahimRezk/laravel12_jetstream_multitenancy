...
// ()=> !empty(rowSelection.value) ? data.map(item => selectedRowsData.value.push))
...
const columns = [
...

    {
        ...
        cell: ({ row }) =>
            h("div", { class: "capitalize" }, row.original.subscriptions.at(-1).status),
    },
    {
        accessorKey: "plan",
        header: "plan",
        ...
        cell: ({ row }) => {
            const actions = [
                ...
                },
                {
                    ...
                    show: row.original.subscriptions.at(-1).status != 'cancelled'
                }
            ]
            ...
            return h(DropdownAction, {
                ...
                onChangeSubscription: (rowData) => handleChangeSubscription(rowData),
                onCancelSubscription: (rowData) => handleCncelSuvbscription(rowData),
            });
        },
    },
];
...
const checkAllItems = () => {
    checkedItems.value = []
    Object.keys(rowSelection.value).forEach((key) => (checkedItems.value.push(data.value[key]['id'])))  ;

    itemsToBeDeleted.value = checkedItems.value
    checkedItems.value.length > 0 ? showAlertModal.value = true : showAlertModal.value = false
};
...
<template>
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="w-full p-2 rounded bg-background border">
                <div class="flex gap-2 items-center py-4">
                    <AddNewTenant :plans="props.plans" :item="tenant" :isDialogOpen @close="isDialogOpen = false" :action />
                    <Input
                        class="max-w-sm"
                        placeholder="Filter tenant name..."
                        :model-value="
                            table.getColumn('tenancy_db_name')?.getFilterValue()
                        "
                        @update:model-value="
                            table
                                .getColumn('tenancy_db_name')
    ...
    <AlertDialogContent >
      ...
      <AlertDialogFooter>
        ...
        <AlertDialogAction @click="cancelTenants"  >Continue</AlertDialogAction>
      </AlertDialogFooter>
    </AlertDialogContent>
  </AlertDialog>
